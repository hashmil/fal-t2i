...
if not fal_key:
    raise ValueError(
        "FAL_KEY not found in environment variables. Please check your .env file.")
...
with gr.Blocks() as demo:
    gr.Markdown("# Fal.ai API Text-to-Image Generator")
    with gr.Row():
        with gr.Column():
            ...
            num_inference_steps = gr.Slider(
                minimum=1, maximum=50, value=28, step=1, label="Number of Inference Steps")
            seed = gr.Number(label="Seed (-1 for random)", value=-1)
            guidance_scale = gr.Slider(
                minimum=1, maximum=20, value=3.5, step=0.1, label="Guidance Scale")
            num_images = gr.Slider(minimum=1, maximum=4,
    ...
    submit_button.click(
        fn=generate_image,
        inputs=[model_choice, prompt_input, image_size, num_inference_steps,
                seed, guidance_scale, num_images, enable_safety_checker],
